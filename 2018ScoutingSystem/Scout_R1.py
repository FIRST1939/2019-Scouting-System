...
OOPS_state = IntVar()
OOPS_state.set(0)
OOPS = Checkbutton(tab4, text='I could not see or something else went wrong', var= OOPS_state)
OOPS.grid(column=0, row= 8, columnspan=3)
...
combo['values']= ( "Did not Try", "Center", "Side", "Used Ramp", "Failed", "Attempt") 
combo.current(0)
...
combo2['values']= ("Did not use cubes", "Only used cube they started with","Received cubes from exchange/portal", "Picked up cubes from ground", "Picked up cubes from ground and recived cubes from portal/exchange")
combo2.current(0)
...
combo3['values']= ("Can score on high scale", "Can score on medium scale", "Can score on low scale", "Can score on switch", "Can score on exchange", "Can not/did not use cubes")
combo3.current(5)
...
combo4['values']= ("Did Not", "less than 30 secs", "30 secs to 1 minute", "more than 1 min")
combo4.current(0)
...
CSS = Label(tab1, text=CSS_Var.get(), textvariable = CSS_Var)
CSSBP = Button(tab1, text='+', command=CSSP1)
CSSBM = Button(tab1, text='-', command=CSSM1)
...
#send button
def clicked():
   messagebox.showinfo('Sent', 'Your scouting data was sent. Find your robot in the next round. Please remember to always be gracious and professional at competions! Do NOT click this button again until the end of the next round. If you do you will be fed to the Killer Rabbot.')
   if OOPS_state.get() == 0:
      sendToDatabase()
   if OOPS_state.get() == 1:
      badData()
